...
const modelMetadataById: Record<
    string,
    { name: string; license: string; tags: string[]; lastUpdated: string; url: string }
> = {
    'nebula-7b': {
...
const metadata = computed(() => {
    ...
    const mapped = modelMetadataById[id]
...
const loadContent = async () => {
    ...
    if (path in modules) {
        ...
        error.value = 'Model details not found.'
    }
    loading.value = false
}
...
<template>
    <div class="min-h-screen bg-black text-white pt-32 pb-20 relative overflow-hidden">
        <!-- Ambient Background -->
        <div
            class="absolute top-0 left-1/2 -translate-x-1/2 w-[600px] h-[400px] bg-gradient-to-r from-purple-500/20 to-blue-500/20 rounded-full blur-[120px] -z-10 pointer-events-none">
        </div>
